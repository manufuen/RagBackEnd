...
def build_bm25_query(question: str, document_id: str | None = None) -> dict[str, Any]:
    base_query = {
        "multi_match": {
            "query": question,
            "fields": [
...
def build_vector_base_query(document_id: str | None = None) -> dict[str, Any]:
    if not document_id:
        return {
            "match_all": {}
        }
...
def bm25_search(
    ...
    query = {
        "size": top_k,
        "query": build_bm25_query(question, document_id=document_id)
    }
...
def vector_search(
    ...
    query = {
        ...
        "query": {
            "script_score": {
                "query": build_vector_base_query(document_id=document_id),
                "script": {
...
def select_best_document(index_name: str, question: str) -> dict[str, Any] | None:
    """
    Primero busca en todo el índice temático.
    Luego agrupa por documento_id.
    Finalmente elige el documento con mejor puntuación acumulada.
    """
    bm25_hits = bm25_search(index_name, question, top_k=20)
    vector_hits = vector_search(index_name, question, top_k=20)
...
def hybrid_search(
    ...
    selected_document = select_best_document(index_name, question)
